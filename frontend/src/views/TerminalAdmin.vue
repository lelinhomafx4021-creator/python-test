  ...
  <section class="space-y-3">
        ...
        >
          <div class="flex flex-wrap items-start justify-between gap-4">
            ...
            <div class="w-full max-w-[220px] space-y-3">
              ...
              <div
                v-else
                class="rounded-lg border border-dashed border-slate-300 px-3 py-6 text-center text-[12px] text-slate-400"
              >
                未上传付款截图
              </div>
              ...
              <div class="flex gap-2">
                <button
                  class="flex-1 rounded-lg bg-emerald-600 px-3 py-2 text-[12px] text-white transition hover:bg-emerald-500"
                  @click="reviewVip(app, 'approve')"
                >
                  通过
                </button>
                <button
                  class="flex-1 rounded-lg bg-rose-600 px-3 py-2 text-[12px] text-white transition hover:bg-rose-500"
                  @click="reviewVip(app, 'reject')"
                >
                  驳回
                </button>
              </div>
      ...
      <div v-if="pagedUsers.length">
        ...
        >
          ...
          <div class="text-slate-600">
            {{ user.aiChatUsed || 0 }}/{{ user.aiChatLimit || 0 }}
          </div>
          <div class="text-slate-600">
    ...
    >
      ...
      <div
        ...
        class="px-4 py-8 text-[13px] text-slate-500"
      >
        正在读取该用户的持仓与委托记录...
      </div>
        ...
        <div class="grid gap-3 md:grid-cols-3">
          <div class="rounded-lg bg-slate-50 px-4 py-3">
            ...
            <div class="mt-1 text-[20px] font-semibold text-slate-950">
              {{ formatMoney(portfolio.account?.totalAsset) }}
            </div>
          </div>
          <div class="rounded-lg bg-slate-50 px-4 py-3">
            <div class="text-[12px] text-slate-500">
            ...
            <div class="mt-1 text-[20px] font-semibold text-slate-950">
              {{ formatMoney(portfolio.account?.cashBalance) }}
            </div>
          </div>
          <div class="rounded-lg bg-slate-50 px-4 py-3">
            <div class="text-[12px] text-slate-500">
            ...
            <div class="mt-1 text-[20px] font-semibold text-slate-950">
              {{ formatMoney(portfolio.account?.totalPnl) }}
            </div>
          </div>
        </div>
