...
<template>
  <div class="space-y-5">
    ...
    <!-- 数据表格（桌面端） -->
    <section class="hidden overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-sm lg:block">
      <!-- 表头 -->
      <div class="grid grid-cols-[120px_90px_90px_70px_90px_120px_120px_120px_1fr] bg-slate-50 px-5 py-2.5 text-[12px] text-slate-500">
        <div>时间</div>
        <div>类型</div>
        <div>股票代码</div>
        <div>方向</div>
      ...
      <div
        v-else
        class="px-5 py-8 text-center text-[13px] text-slate-500"
      >
        暂无交易记录。
      </div>
    ...
    <section class="space-y-3 lg:hidden">
      <div v-if="transactions.length">
        <div
          v-for="tx in transactions"
          :key="tx.id"
          class="rounded-[24px] border border-slate-200 bg-white px-5 py-4 shadow-sm"
        >
          ...
          <div
            v-if="tx.description"
            class="mt-3 rounded-2xl bg-slate-50 px-3 py-2 text-[12px] leading-6 text-slate-600"
          >
            {{ tx.description }}
          </div>
        </div>
      ...
      <div
        v-else
        class="rounded-[32px] border border-slate-200 bg-white px-6 py-14 text-center shadow-sm"
      >
        ...
        <h4 class="mt-5 text-2xl font-semibold text-slate-950">
          当前没有交易记录
        </h4>
