    ...
    <main class="mx-auto max-w-[560px] px-6 py-8">
      <div v-if="step === 'info'">
        <div class="rounded-2xl border border-slate-200 bg-white p-6">
          <div class="text-[22px] font-bold text-slate-950">
            专业版权益
          </div>
      ...
      <div v-if="step === 'pay'">
        <div class="rounded-2xl border border-slate-200 bg-white p-6">
          <div class="text-[18px] font-bold text-slate-950">
            扫码付款
          </div>
          ...
          <div class="mt-6 flex justify-center">
            <img
              ...
              class="h-[320px] rounded-2xl border border-slate-200"
              alt="收款码"
            >
          </div>
        ...
        <div class="mt-4 flex gap-3">
          <button
            class="flex-1 rounded-xl border border-slate-200 bg-white px-5 py-3 text-[13px] font-medium text-slate-600 transition hover:bg-slate-50"
            @click="step = 'info'"
          >
            返回
          </button>
          <button
            class="flex-1 rounded-xl bg-slate-900 px-5 py-3 text-[13px] font-medium text-white transition hover:bg-slate-800"
            @click="step = 'submit'"
          >
            我已付款，下一步
          </button>
      ...
      <div v-if="step === 'submit'">
        <div class="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div class="mt-5 space-y-4">
            ...
            <div>
              <div class="mb-1.5 text-[12px] text-slate-500">
                付款凭证截图
              </div>
          ...
          <div
            v-if="errorMsg"
            class="mt-4 rounded-lg bg-rose-50 px-3 py-2 text-[12px] text-rose-500"
          >
            {{ errorMsg }}
          </div>
        </div>
      ...
      <div v-if="step === 'done'">
        <div class="rounded-2xl border border-slate-200 bg-white p-8 text-center">
          ...
          <div class="mt-5 text-[20px] font-bold text-slate-950">
            申请已提交
          </div>
        ...
        <button
          class="mt-5 inline-flex w-full items-center justify-center rounded-xl border border-slate-200 bg-white px-6 py-3.5 text-[14px] font-medium text-slate-700 transition hover:bg-slate-50"
