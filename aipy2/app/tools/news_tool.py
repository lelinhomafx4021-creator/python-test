...
def collect_hot_news(limit: int = 12) -> list[dict[str, Any]]:
    ...
    if cx_df is not None:
        for _, row in cx_df.head(limit).iterrows():
            ...
            title = safe_text(row.get("summary"))
            ...
            result.append(
                {
                    "title": title,
                    "summary": title,
                    "tag": safe_text(row.get("tag"), "市场动态"),
                    "source": "财新数据通",
                    "url": url,
                    "publishedAt": None,
                }
    ...
    if em_df is not None:
        for _, row in em_df.iterrows():
            url = safe_text(row.get("新闻链接"))
            title = safe_text(row.get("新闻标题"))
            unique_key = url or title
            if not unique_key or unique_key in seen:
                continue
            seen.add(unique_key)
            result.append(
                {
                    "title": title,
                    "summary": safe_text(row.get("新闻内容"), title)[:140],
                    "tag": safe_text(row.get("关键词"), "股票资讯"),
                    "source": safe_text(row.get("文章来源"), "东方财富"),
                    "url": url,
                    "publishedAt": format_time(row.get("发布时间")),
                }
            )
            if len(result) >= limit:
                break
